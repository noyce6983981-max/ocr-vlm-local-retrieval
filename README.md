...
V17-Compositional-80 在冻结 K=3、`full_query` 阈值 0.51 后，只运行一次 40 条复合视觉留出集。两名审核者完整盲审 40×20 个候选，14 条分歧或不确定项由独立第三审核者裁决；留出标签没有使用模型辅助。

| 池条件指标 | V16 | V17 |
|---|---:|---:|
| 端到端正确率 | 15.0% | **47.5%** |
| 池内无相关候选误接受率 | 62.5% | **37.5%** |
| 池内无相关候选拒绝率 | 37.5% | **62.5%** |
