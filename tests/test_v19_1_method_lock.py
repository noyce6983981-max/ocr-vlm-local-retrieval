...
from scripts.freeze_v19_1_method import LOCKED_PATHS, locked_hashes, verify_lock


def test_v19_1_method_lock_detects_changed_file(tmp_path: Path) -> None:
    ...
    assert verify_lock(payload, tmp_path) == [
        f"SHA-256 mismatch: {LOCKED_PATHS[0]}"
    ]


def test_v19_1_method_lock_reports_missing_file(tmp_path: Path) -> None:
    payload = {"locked_files": {}}
    errors = verify_lock(payload, tmp_path)
    assert len(errors) == len(LOCKED_PATHS)
