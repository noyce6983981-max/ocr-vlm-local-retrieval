"""Human review UI for the 12 V19.1 holdout query families only."""
...
DRAFT_PATH = PRIVATE_DIR / "holdout_query_drafts_machine.json"
...
ROLE_LABELS = {
    "answerable_positive": "可回答正例（目标页完整满足）",
    "paraphrase_positive": "自然改写正例（目标页完整满足）",
    "single_condition_hard_negative": "单条件强负例（只改变一个条件）",
    "unanswerable_neighbor": "近邻无答案（近邻页也不完整满足）",
}
...
def load_families() -> list[dict[str, Any]]:
    ...
    if lock.get("status") != "locked_after_human_reviewed_development_before_holdout":
        raise ValueError("V19.1 method lock is not ready")
    if lock.get("holdout_scored") is not False:
        raise ValueError("holdout has already been scored")
    sources = [row for row in _read_jsonl(FAMILY_PATH) if row["split"] == "holdout"]
    draft_payload = json.loads(DRAFT_PATH.read_text(encoding="utf-8-sig"))
    if draft_payload.get("method_lock_verified_before_authoring") is not True:
        raise ValueError("holdout drafts were not created after method lock")
    if draft_payload.get("retrieval_run") is not False:
        raise ValueError("holdout retrieval must not run before query freeze")
    if draft_payload.get("candidate_scored") is not False:
        raise ValueError("holdout candidate must not be scored before query freeze")
    drafts = {str(row["family_id"]): row for row in draft_payload.get("families", [])}
    if len(sources) != 12 or {str(row["family_id"]) for row in sources} != drafts.keys():
        raise ValueError("expected 12 matched V19.1 holdout families")
    return [{**source, **drafts[str(source["family_id"])]} for source in sources]
...
def _review_path(reviewer_id: str) -> Path:
    ...
    return PRIVATE_DIR / f"{reviewer_id}_holdout_query_reviews.jsonl"
...
def _save_review(
    ...
    *,
    ...
    if not changed_condition_kind.strip():
        raise ValueError("必须填写强负例唯一改变的必要条件")
    ...
    reviews[family_id] = {
        ...
        "decision": "human_approved_holdout_family",
        "changed_condition_kind": changed_condition_kind.strip(),
        "query_texts": cleaned,
        "notes": notes.strip(),
        "retrieval_seen_during_review": False,
        "candidate_score_seen_during_review": False,
    }
...
def render() -> None:
    st.set_page_config(page_title="V19.1 留出查询审核", layout="wide")
    st.title("V19.1 必要条件完整性：留出查询审核")
    st.caption(
        "方法已经冻结；本页只审核 12 个独立 holdout 家族。"
        "尚未运行检索、验证器或候选评分。"
    )
    ...
    selection_key = "v19_1_selected_holdout_family"
    advance_key = "v19_1_advance_to_holdout_family"
    ...
    for role in ROLES:
        query_texts[role] = st.text_area(
            ...
            key=f"holdout_{selected_id}_{role}",
            height=75,
        )
    changed_condition_kind = st.text_input(
        ...
        key=f"holdout_{selected_id}_condition_kind",
    )
    notes = st.text_area(
        "备注（可选）",
        value=str(existing.get("notes", "") if existing else ""),
        key=f"holdout_{selected_id}_notes",
        height=65,
    )
    attested = st.checkbox(
        "我已核对：两条正例完整满足；强负例只改变一个条件；近邻查询确实无答案",
        key=f"holdout_{selected_id}_attested",
    )
    if st.button("保存本家族", type="primary", disabled=not attested):
        try:
            ...
            st.rerun()
    if len(reviews) == 12:
        st.success("12 个 holdout 家族已全部完成；可冻结查询后执行一次性评测。")
